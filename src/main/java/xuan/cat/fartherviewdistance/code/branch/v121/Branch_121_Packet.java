...
public final class Branch_121_Packet implements BranchPacket {

    private final Branch_121_PacketHandleChunk handleChunk = new Branch_121_PacketHandleChunk();
    private final Branch_121_PacketHandleLightUpdate handleLightUpdate = new Branch_121_PacketHandleLightUpdate();
    ...
    public Consumer<Player> sendChunkAndLight(final BranchChunk chunk, final BranchChunkLight light, final boolean needTile,
        ...
        final LevelLightEngine levelLight = ((Branch_121_Chunk) chunk).getLevelChunk().level.getLightEngine();
        serializer.writeInt(chunk.getX());
        serializer.writeInt(chunk.getZ());
        this.handleChunk.write(serializer, ((Branch_121_Chunk) chunk).getLevelChunk(), needTile);
        this.handleLightUpdate.write(serializer, (Branch_121_ChunkLight) light);
        ...
        final ClientboundLevelChunkWithLightPacket packet = new ClientboundLevelChunkWithLightPacket(
                ((Branch_121_Chunk) chunk).getLevelChunk(), levelLight, null, null, false);
        try {
            // 適用於 paper
