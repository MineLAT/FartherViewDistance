...
public final class Branch_121_PacketHandleLightUpdate {

    public Branch_121_PacketHandleLightUpdate() {}

    public void write(final FriendlyByteBuf serializer, final Branch_121_ChunkLight light) {
        ...
        for (int index = 0; index < light.getArrayLength(); ++index) {
            Branch_121_PacketHandleLightUpdate.saveBitSet(light.getSkyLights(), index, notSkyEmpty, isSkyEmpty, dataSky);
            Branch_121_PacketHandleLightUpdate.saveBitSet(light.getBlockLights(), index, notBlockEmpty, isBlockEmpty, dataBlock);
        }
    ...
    private static void saveBitSet(final byte[][] nibbleArrays, final int index, final BitSet notEmpty, final BitSet isEmpty,
        ...
        if (nibbleArray != Branch_121_ChunkLight.EMPTY) {
            if (nibbleArray == null) {
                isEmpty.set(index);
            } else {
                notEmpty.set(index);
